...
def analyze_bin_methods(data):
    ...
    # Get all column names except ID (column 0) and target (column 17)
    all_columns = data.columns.tolist()
    feature_columns = [col for i, col in enumerate(all_columns) if i != 7]
    ...
    for column in feature_columns:
        ...
        print(f"Column {column_index:2d} ({column[:7]:<20}): ", end="")
...
def main():
    ...
    print("Loading Parkinsons dataset...")

    # Load data
    data = load_parkinsons_data('rice.csv')
    ...
    print(f"Target variable (column 17) distribution:")
    print(data.iloc[:, 7].value_counts())
    ...
    # Save results to CSV
    df_results.to_csv('bin_analysis_results.csv', index=False)
    print(f"\nResults saved to 'bin_analysis_results.csv'")
